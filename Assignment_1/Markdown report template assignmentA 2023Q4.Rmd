...
synthetic <- data.frame(
  ...
  sentiment = sample(-3:5, size = 300, replace = TRUE)
)
head(synthetic)
...
ggplot() +
  ...
  geom_point(data = mean_data, aes(x = Candidate, y = score, fill = "Mean"),
             shape = 23, size = 3) +
...
The table shows that Model 2 provides a significantly better fit to the data compared to Model 1 (Pr(>F) = 0.0008022). This suggests that the candidate has a significant impact on the sentiment variable.
The lower the AIC value, the better the model. Model 2 (AIC = 1421.824) has a lower AIC value compared to Model 1 (AIC = 1432.224). This suggests that Model 2 provides a better fit to the data, considering both the explanatory power and the complexity of the model.

Based on these results, it can be concluded that the analysis with synthetic data successfully reproduces the coefficients of the linear model used to generate the data. The p-value in the ANOVA table indicates that the candidate has a significant effect on the "sentiment" variable. The AIC values confirm that Model 2, which includes the candidate, provides a better fit to the data compared to Model 1.
